...
const doLocationRow = async (rows, index, limit) => {
  ...
  if (currentRow.trim().length > 0 && !isNaN(currentRow.split(',')[0])) {
    const [lat, long, start, end] = currentRow.split(',');
    const [nextLat, nextLong, nextStart, nextEnd] = nextRow.split(',');
    const { duration, gap } = getDuration(start, end, nextStart);
    const outputRowArray = [lat.trim(), long.trim(), start.trim(), end.trim(), duration];
...
const output = [
  `Lat,Long,Start,End,Duration(minutes),FirstLine,SecondLine,Area,City,County,Region,State,Postcode,Country`,
];
// const output = [
